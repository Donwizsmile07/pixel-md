...
// ===== IMPORTANT: imports fixed (added missing helpers) =====
const {
  smsg,
  imageToWebp,
  videoToWebp,
...
const startBotz = async() => {
  ...
  */
  ...
  // ===== fixed sendContact: proper strings / backticks and using global.botname fallback =====
  donwiz.sendContact = async (jid, kon, desk = "Developer Bot", quoted = '', opts = {}) => {
    let list = []
    for (let i of kon) {
      list.push({
        displayName: global.botname || "PixelBot",
        vcard:
          'BEGIN:VCARD\n' +
          'VERSION:3.0\n' +
          `N:;${global.botname || "PixelBot"};;;\n` +
          `FN:${global.botname || "PixelBot"}\n` +
          'ORG:null\n' +
          'TITLE:\n' +
          `item1.TEL;waid=${i}:${i}\n` +
          'item1.X-ABLabel:Ponsel\n' +
          `X-WA-BIZ-DESCRIPTION:${desk}\n` +
          `X-WA-BIZ-NAME:${global.botname || "PixelBot"}\n` +
          'END:VCARD'
      })
    }
  ...
  // ===== fixed sendImageAsSticker: valid regex, proper split, getBuffer used, writeExifImg imported =====
  ...
  // ===== fixed sendVideoAsSticker: valid regex, proper split, getBuffer used, writeExifVid imported =====
  ...
  // ===== fixed sendMedia: dynamic key using variable (pase) correctly =====
  ...
  // ===== fixed getFile: corrected regex, proper getBuffer usage, safe fallback and saving =====
  donwiz.getFile = async (PATH, save) => {
    ...
    // if getBuffer returned an axios Buffer-like data, ensure it's a Buffer
